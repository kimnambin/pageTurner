...
const order = async (req, res) => {
  ...
  if (authorization instanceof jwt.TokenExpiredError) {
    ...
    sql = `SELECT cart_book_id , quantity FROM cartItems WHERE cart_id IN (?)`;
...
const deleteCartItems = async (conn, items) => {
  let sql = `DELETE FROM cartItems WHERE cart_id IN (?)`;
...
const get_order_detail = async (req, res) => {
  ...
  FROM orderedBooks LEFT JOIN books ON orderedBooks.book_id = books.id 
  WHERE order_id = ?`;
